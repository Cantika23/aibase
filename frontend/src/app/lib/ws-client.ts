...
export class WSClient extends WSEventEmitter {
    ...
    return new Promise((resolve) => {
      ...
      this.ws!.onmessage = async (event) => {
        ...
        try {
          ...
          console.log('WSClient: Parsed message:', message.type, message.id, message.data);
          await this.handleMessage(message);
        } catch (error) {
          console.error('WSClient: Failed to parse message:', error, 'Raw data:', event.data);
  ...
  public send(message: WSMessage): void {
    console.log('WSClient: Attempting to send message:', message.type, message.id);
    ...
    if (this.isConnected()) {
      ...
      console.log('WSClient: Not connected, queuing message. Queue length:', this.messageQueue.length);
  ...
  private setState(newState: ConnectionState['status']): void {
    if (this.state.status !== newState) {
      ...
      this.state.messageCount = this.stats.messagesSent + this.stats.messagesReceived;
