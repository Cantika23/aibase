...
initialize_ports() {
    # Load saved ports from .dev-ports if exists
    load_saved_ports

    # Determine backend port to use
    if [ -n "$BACKEND_PORT" ]; then
        # We have a saved port, check if it's available
        if is_port_in_use $BACKEND_PORT; then
            warn "Saved backend port $BACKEND_PORT is in use, finding alternative..."
        ...
        # No saved port, check if default is available
    ...
    # Frontend port is always default (Vite will auto-adjust if needed)
    FRONTEND_PORT=$DEFAULT_FRONTEND_PORT
    success "Using frontend port: $FRONTEND_PORT (Vite may auto-adjust if in use)"

    # Save the backend port to .dev-ports
    cat > "$PORTS_FILE" << EOF
