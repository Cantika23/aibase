...
initialize_ports() {
    ...
    if [ -z "$FRONTEND_PORT" ]; then
        ...
        success "Using saved frontend port: $FRONTEND_PORT (Vite may auto-adjust if in use)"
    fi
...
if ! kill -0 $BACKEND_PID 2>/dev/null; then
    echo ""
    error_exit "Backend failed to start. Port $BACKEND_PORT might be in use.
Try deleting .dev-ports and running dev.sh again."
fi
